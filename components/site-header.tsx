...
export function SiteHeader() {
  ...
  const { studyPrograms, courses } = useData();
  const isProgram = pathname[1] === "programs"
  ...
  const segments = pathname
    .map((segment, index) => {
      ...
      if (isUUID(segment)) {
        const displayName = isProgram ? (studyPrograms.find(p => p.id === segment)?.name || "Loading...") : (courses.find(c => c.id === segment)?.name || "Loading...");
        return (
